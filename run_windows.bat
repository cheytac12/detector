...
if !MINOR! GEQ 12 (
    ...
    "!PIP!" install --quiet "tensorflow>=2.16.0" "mediapipe==0.10.21" "opencv-python==4.9.0.80" "numpy==1.26.4" "Pillow==10.4.0" "scikit-learn==1.4.2" "scipy==1.13.1" "protobuf>=3.20.0,<4.0.0"
) else (
    "!PIP!" install --quiet -r requirements.txt
    if errorlevel 1 (
        echo   Bulk install failed - falling back to individual installs...
        for %%p in ("tensorflow==2.15.1" "mediapipe==0.10.21" "opencv-python==4.9.0.80" "numpy==1.26.4" "Pillow==10.4.0" "scikit-learn==1.4.2" "scipy==1.13.1" "protobuf>=3.20.0,<4.0.0") do (
            "!PIP!" install --quiet %%p
        )
    )
)
...
if errorlevel 1 (
    ...
    "!PIP!" install --quiet --force-reinstall "mediapipe==0.10.21" "numpy==1.26.4" "protobuf>=3.20.0,<5.0.0"
    "!PY!" -c "import mediapipe; import cv2; import numpy" >nul 2>&1
    if errorlevel 1 (
